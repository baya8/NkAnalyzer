 ...
 */
@Component
public class CreateMatcher implements Tasklet {

	private final static Logger logger = LoggerFactory.getLogger(CreateMatcher.class);
	...
	public RepeatStatus execute(StepContribution contribution, ChunkContext chunkContext) throws Exception {
		...
		// deleteDuplicate(originalMatcherList);
	 ...
	 * ストリーム処理へ実装変更したため、当メソッドは使用しない。
	 *
	 * @param matcherList
	 */
	@SuppressWarnings("unused")
	private void deleteDuplicate(List<FeatureMatcher> matcherList) {

		final List<FeatureMatcher> tmp = new ArrayList<>(matcherList.size());

		// TODO 処理が終わらん。ストリーム使わんとヤバシ。現在、50万×50万回ループが回る。
		for (FeatureMatcher matcher : matcherList) {

			FeatureMatcher matchElement = null;

			for (FeatureMatcher f : tmp) {
	...
	private void deleteDuplicateReNew(List<FeatureMatcher> matcherList) {
		...
		Map<FeatureMatcher, Integer> asdf =
				matcherList
						.stream()
						.collect(
	...
	private void sortCount(List<FeatureMatcher> list) {
		...
		//		List<FeatureMatcher> asdf = list.stream().distinct().collect(Collectors.toList());
		Collections.sort(list, new Comparator<FeatureMatcher>() {
			@Override
			public int compare(FeatureMatcher o1, FeatureMatcher o2) {
